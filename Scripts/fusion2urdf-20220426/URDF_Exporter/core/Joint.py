...
class Joint:
    def __init__(self, name, xyz, rpy, axis, parent, child, joint_type, upper_limit, lower_limit):
        ...
        rpy: [x, y, z]
            rotation around the axis (roll, pitch, yaw)
        axis: [x, y, z]
    ...
    def make_joint_xml(self, hasOrigin = True):
        """
        ...
        if hasOrigin:
            ...
            origin.attrib = {
                ...
                'rpy': (' '.join([str(_) for _ in self.rpy]) if self.rpy else '0 0 0')
            }
...
def make_joints_dict(root, msg):
    ...
    ----------
    joints_dict:
        {name: {type, axis, upper_limit, lower_limit, parent, child, xyz, rpy}}
    msg: str
        Tell the status
    """
    ...
    for joint in root.joints:
        ...
        if joint_type == 'revolute':
            ...
            if 'wheel' in joint.occurrenceOne.component.name and 'base_link' in joint.occurrenceTwo.component.name:
                #NOTE Due to a bug, one of the axis for one of the wheels is negative. They should be positive for both wheels
                joint_axis_override = [0.0, 1.0, 0.0]
                print('ATTENTION! Manual override for revolute joint axis, from %s to %s' % (joint_dict['axis'], joint_axis_override))
                joint_dict['axis'] = joint_axis_override
            ...
            if 'camera_rear_lens' in joint.occurrenceOne.component.name and 'camera_rear' in joint.occurrenceTwo.component.name:
                #NOTE Make sure that the rear camera is facing backwards
                joint_rpy_override = [0, 0, -3.141592]
                print('ATTENTION! Manual override for camera_rear_lens rpy, from %s to %s' % (joint_dict['rpy'], joint_rpy_override))
                joint_dict['rpy'] = joint_rpy_override
        ...
        try:
            ...
            try:
                ...
                msg = joint.name + " doesn't have a joint origin. Please set it and run again."
                break
