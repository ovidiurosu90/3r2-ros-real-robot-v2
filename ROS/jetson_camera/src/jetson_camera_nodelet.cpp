...
private:
    static std::string gstreamer_pipeline (int sensor_id, int capture_width, int capture_height,
        int display_width, int display_height, int framerate, int flip_method
    ) {
        return "nvarguscamerasrc sensor_id=" + std::to_string(sensor_id) + " ! video/x-raw(memory:NVMM), width=(int)" +
            std::to_string(capture_width) + ", height=(int)" + std::to_string(capture_height) +
            ", format=(string)NV12, framerate=(fraction)" + std::to_string(framerate) + "/1 ! nvvidconv flip-method=" +
            std::to_string(flip_method) + " ! video/x-raw, width=(int)" + std::to_string(display_width) + ", height=(int)" +
    ...
    void onInit() override
    {
        ...
        std::string camera_name = nh_priv_.param<std::string>("camera_name", "camera");

        std::string url = nh_priv_.param<std::string>("camera_info_url", "");

        camera_info_manager::CameraInfoManager info_manager(nh_, camera_name, url);
        ...
        NODELET_INFO("jetson_camera: Starting node with the following parameters:\n"\
                     "sensor_id: %d, camera_name: %s\n"\
                     "capture width: %d, capture height: %d\n"\
                     "published width: %d, published height: %d\n"\
                     "requested framerate: %d, flip method: %d\n",
                     sensor_id_, camera_name.c_str(), cap_width_, cap_height_, width_, height_, framerate_, flip_method_);
